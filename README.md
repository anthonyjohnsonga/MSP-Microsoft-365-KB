...
[![Articles](https://img.shields.io/badge/articles-33-0078D4?style=flat-square)](#-article-index)
[![Product Areas](https://img.shields.io/badge/product%20areas-13-5B2D90?style=flat-square)](#-article-index)
...
|---|---|---|---|
| **M365 Apps** | [Microsoft 365 Apps for Mac — Stale Tenant Activation Fix](./Microsoft%20365%20Apps/Microsoft%20365%20Apps%20for%20Mac%20Stale%20Tenant%20Activation%20Fix.md) | Office for Mac stuck in view-only after a tenant migration: diagnosing the stale `com.microsoft.office` tenant pointer, surgical vs. plist-nuke cleanup, ADAL blocks and keychain, verifying via `licensingV2.plist`, the Windows registry equivalent, and the post-migration sweep | `microsoft-365-apps` `macos` `activation` `licensing` `tenant-migration` `troubleshooting` |
...
| **Intune** | [Deploy Bitwarden Extension to Microsoft Edge](./Microsoft%20Intune/Browser%20Extensions/Bitwarden%20Edge%20Extension%20Deployment%20Runbook.md) | User-scoped Settings Catalog policy that silently installs Bitwarden in Edge, prevents removal, forces the toolbar icon to remain visible, and covers validation and rollback | `intune` `edge` `bitwarden` `browser-extension` `settings-catalog` `password-manager` |
| **Intune** | [Deploy PDFgear 2.1.20 with Microsoft Intune](./Microsoft%20Intune/Apps/PDFgear%20Intune%20Win32%20Deployment%20Runbook.md) | Package and deploy PDFgear 2.1.20 as an Intune Win32 app, including installer validation, commands, detection choices, pilot checks, supersedence, and troubleshooting | `intune` `win32-app` `pdfgear` `application-deployment` `detection-rules` `supersedence` |
...
| **Edge** | [Enterprise Sync Troubleshooting](./Microsoft%20Edge/Microsoft%20Edge%20Enterprise%20Sync%20Troubleshooting.md) | Resolve “Sync isn't available for this account” by checking Edge policies, licensing, Rights Management, IPCv3, onboarding controls, ESR, and client sync state | `edge` `enterprise-sync` `rights-management` `aipservice` `entra` `troubleshooting` |
...
| **OneDrive** | [Launch Failure Diagnosis with Process Monitor](./Microsoft%20OneDrive/Microsoft%20OneDrive%20Launch%20Failure%20Diagnosis%20with%20Process%20Monitor.md) | OneDrive starts and instantly exits with no dialog, event log entry, or ODL logs: ProcMon filter setup, working backwards from `Process_Exit`, exit-status decoding, noise to ignore, the `DisableFileSyncNGSC` policy kill switch, and the two separate OneDrive policy registry paths | `onedrive` `procmon` `sysinternals` `group-policy` `intune` `troubleshooting` |
